...
impl SimulatedExchange {
    ...
    pub fn process_instrument(&mut self, instrument: InstrumentAny) {
        ...
        if let Some(matching_engine) = self.matching_engines.get_mut(&instrument_id) {
            match matching_engine.update_instrument(instrument) {
                Ok(()) => log::info!("Updated instrument {instrument_id} on matching engine"),
                Err(e) => log::error!("Failed to update instrument {instrument_id}: {e}"),
            }
