...
fn extract_chainlink_data_factory(
    ...
    match factory.extract::<ChainlinkDataClientFactory>(py) {
        ...
        Err(err) => Err(to_pyvalue_err(format!(
            "Failed to extract ChainlinkDataClientFactory: {err}"
        ))),
    }
}
...
fn extract_chainlink_data_config(
    ...
    match config.extract::<ChainlinkDataClientConfig>(py) {
        ...
        Err(err) => Err(to_pyvalue_err(format!(
            "Failed to extract ChainlinkDataClientConfig: {err}"
        ))),
    }
}
...
pub fn chainlink(_: Python<'_>, m: &Bound<'_, PyModule>) -> PyResult<()> {
    register_custom_data_json::<ChainlinkData>().map_err(to_pyruntime_err)?;
    ...
    registry
        ...
        .map_err(|err| {
            to_pyruntime_err(format!(
                "Failed to register Chainlink data factory extractor: {err}"
            ))
        })?;
    ...
    registry
        ...
        .map_err(|err| {
            to_pyruntime_err(format!(
                "Failed to register Chainlink data config extractor: {err}"
            ))
        })?;
