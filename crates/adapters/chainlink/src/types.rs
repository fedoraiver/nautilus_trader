...
impl ChainlinkData {
    ...
    pub fn from_ws_report(response: &WebSocketReport, ts_init: UnixNanos) -> anyhow::Result<Self> {
        ...
        let full_payload = hex::decode(payload_hex)
            .map_err(|err| anyhow::anyhow!("Failed to decode Chainlink full_report hex: {err}"))?;
        let (_context, payload_blob) = decode_full_report(&full_payload).map_err(|err| {
            anyhow::anyhow!("Failed to decode Chainlink full report payload: {err}")
        })?;
        let decoded = ReportDataV3::decode(&payload_blob).map_err(|err| {
            anyhow::anyhow!("Failed to decode Chainlink report payload as v3: {err}")
        })?;
