...
use nautilus_model::{
    data::{CustomData, Data, DataType, ensure_custom_data_json_registered},
...
impl ChainlinkDataClient {
    ...
    pub fn new(client_id: ClientId, config: ChainlinkDataClientConfig) -> anyhow::Result<Self> {
        ...
        ensure_custom_data_json_registered::<ChainlinkData>()?;
    ...
    fn send_feed_update(&mut self) -> anyhow::Result<()> {
        ...
        if let Some(tx) = &self.command_tx {
            ...
            tx.send(WorkerCommand::SetFeedIds(feed_ids))
                .map_err(|e| anyhow::anyhow!("Failed to update Chainlink subscriptions: {e}"))
        } else {
            Ok(())
        }
    }
...
async fn run_worker(
    ...
    loop {
        ...
        let parsed_feed_ids = match feed_ids
            .iter()
            .map(|feed_id| {
                ID::from_hex_str(feed_id).map_err(|e| {
                    anyhow::anyhow!(
                        "Invalid Chainlink feed ID `{feed_id}` reached worker loop: {e}"
                    )
                })
            })
        ...
        let mut stream = match Stream::new(&resolved.sdk_config, parsed_feed_ids).await {
            Ok(stream) => stream,
            Err(e) => {
                log::warn!("Failed to create Chainlink stream: {e}");
                is_connected.store(false, Ordering::Release);
        ...
        if let Err(e) = stream.listen().await {
            log::warn!("Failed to start Chainlink stream listener: {e}");
            is_connected.store(false, Ordering::Release);
                    ...
                    match result {
                        Ok(report) => {
                            ...
                            match ChainlinkData::from_ws_report(&report, ts_init) {
                                ...
                                Err(e) => {
                                    log::error!("Failed to decode Chainlink report: {e}");
                                }
                            }
                        }
                        Err(e) => {
                            log::warn!("Chainlink stream read failed: {e}");
                            should_retry = true;
                            break;
                        }
                    }
...
fn extract_feed_ids(
    ...
    if data_type.type_name() != CHAINLINK_DATA_TYPE {
        anyhow::bail!(
            "ChainlinkDataClient only supports `{CHAINLINK_DATA_TYPE}` subscriptions, was `{}`",
            data_type.type_name()
        );
    }
            ...
            Value::Array(feed_ids) => {
                for feed_id in feed_ids {
                    let feed_id = feed_id.as_str().ok_or_else(|| {
                        anyhow::anyhow!(
                            "Chainlink `{FEED_IDS_KEY}` entries must be strings, received `{feed_id}`"
                        )
                    })?;
                    target.insert(canonicalize_feed_id(feed_id)?);
                }
...
mod tests {
    ...
    fn test_extract_feed_ids_from_metadata() {
        ...
        let data_type = DataType::new(CHAINLINK_DATA_TYPE, Some(metadata), None);
