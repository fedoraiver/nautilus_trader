...
use std::{collections::BTreeSet, env, fmt::Debug, time::Duration};
...
impl Debug for ChainlinkDataClientConfig {
...
impl Debug for ResolvedChainlinkConfig {
    fn fmt(&self, f: &mut std::fmt::Formatter<'_>) -> std::fmt::Result {
...
impl ChainlinkDataClientConfig {
    ...
    pub(crate) fn resolve(&self) -> anyhow::Result<ResolvedChainlinkConfig> {
        ...
        let sdk_config = ChainlinkSdkConfig::new(api_key, api_secret, api_base_url, ws_base_url)
            ...
            .build()
            .map_err(|e| anyhow::anyhow!("Failed to build Chainlink SDK config: {e}"))?;
